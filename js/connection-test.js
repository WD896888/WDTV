...
(function () {
    ...
    async function testSource(key) {
        ...
        const searchUrl = buildVodApiUrl(api.api, API_CONFIG.search.path) + encodeURIComponent(TEST_QUERY);
        ...
        if (searchJson && Array.isArray(searchJson.list) && searchJson.list.length > 0) {
            ...
            if (detailMode === 'api' || detailMode === 'auto') {
                const dUrl = buildVodApiUrl(api.api, API_CONFIG.detail.path) + vodId;
                let ok = false;
                let note = '';
                // 直连
                try {
