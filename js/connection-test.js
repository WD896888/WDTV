...
(function () {
    ...
    async function testSource(key) {
        ...
        let proxiedUrl = '';
        try {
            proxiedUrl = window.ProxyAuth && window.ProxyAuth.addAuthToProxyUrl
                ? await window.ProxyAuth.addAuthToProxyUrl(PROXY_URL + encodeURIComponent(apiUrl))
                : PROXY_URL + encodeURIComponent(apiUrl);
            push('conn-step', `代理地址（含鉴权）: ${proxiedUrl}`);
        } catch (authErr) {
            push('conn-warn', `附加鉴权参数失败: ${describeError(authErr).join(' / ')}`);
            proxiedUrl = PROXY_URL + encodeURIComponent(apiUrl);
            push('conn-step', `代理地址（无鉴权回退）: ${proxiedUrl}`);
        }
        ...
        try {
            ...
            if (resp.ok) {
                ...
                if (resp.status === 401) {
                    push('conn-fail', `  → 鉴权失败：代理要求密码鉴权，请检查部署的 PASSWORD 是否与本地登录密码一致`);
                }
