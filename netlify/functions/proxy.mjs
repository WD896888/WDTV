...
async function fetchContentWithType(targetUrl, requestHeaders) {
    ...
    try {
        ...
        // 以二进制读取响应（视频分片、图片等二进制内容绝不能用 text() 读取，否则数据损坏）
        const buffer = Buffer.from(await response.arrayBuffer());
        const contentType = response.headers.get('content-type') || '';
        logDebug(`Fetch success: ${targetUrl}, Content-Type: ${contentType}, Length: ${buffer.length}`);
...
async function processM3u8Content(targetUrl, content) {
    if (content.includes('#EXT-X-STREAM-INF') || content.includes('#EXT-X-MEDIA:')) { logDebug(`Detected master playlist: ${targetUrl}`); return processMasterPlaylist(targetUrl, content); }
    logDebug(`Detected media playlist: ${targetUrl}`); return processMediaPlaylist(targetUrl, content);
}
...
export const handler = async (event, context) => {
    ...
    try {
        ...
        const { buffer, contentType, responseHeaders } = await fetchContentWithType(targetUrl, event.headers);

        // --- Process if M3U8 ---
        if (isM3u8Buffer(buffer, contentType)) {
            logDebug(`Processing M3U8 content: ${targetUrl}`);
            const processedM3u8 = await processM3u8Content(targetUrl, buffer.toString('utf8'));
            ...
            // 二进制安全回传：base64 + isBase64Encoded，避免图片/视频分片被 UTF-8 解码损坏
