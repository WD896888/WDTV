...
export async function onRequest(context) {
    ...
    async function fetchContentWithType(targetUrl) {
        ...
        try {
            ...
            return { buffer, contentType, responseHeaders: response.headers }; // 同时返回原始响应头
    ...
    async function processMasterPlaylist(url, content, recursionDepth, env) {
        ...
        const variantContent = new TextDecoder().decode(variantBuffer);
    ...
    try {
        ...
        // --- 缓存检查 (KV) ---
        // 仅缓存 M3U8 文本；二进制分片交给 Cache-Control 边缘/浏览器缓存，避免 KV 存储膨胀与二进制损坏
        ...
        if (kvNamespace) {
            try {
                const cachedM3u8 = await kvNamespace.get(cacheKey); // 读取缓存的 M3U8 文本
                if (cachedM3u8) {
                    logDebug(`[缓存命中] M3U8: ${targetUrl}`);
                    const processedM3u8 = await processM3u8Content(targetUrl, cachedM3u8, 0, env);
        ...
        // --- 写入缓存 (KV，仅 M3U8 文本) ---
        if (kvNamespace && isM3u8) {
             try {
                 waitUntil(kvNamespace.put(cacheKey, new TextDecoder().decode(buffer), { expirationTtl: CACHE_TTL }));
                 logDebug(`已将 M3U8 内容写入缓存: ${targetUrl}`);
            } catch (kvError) {
                 logDebug(`向 KV 写入缓存失败 (${cacheKey}): ${kvError.message}`);
                 // 写入失败不影响返回结果
            }
        ...
        if (isM3u8) {
            ...
            const processedM3u8 = await processM3u8Content(targetUrl, new TextDecoder().decode(buffer), 0, env);
            return createM3u8Response(processedM3u8);
        } else {
            ...
            return createResponse(buffer, 200, finalHeaders);
        }
